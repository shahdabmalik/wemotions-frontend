    ...
    return (
        <div>
            <div className="flex items-center justify-between">
                ...
                <Link to={"/pages/add"} className="bg-purple-600 text-white font-semibold px-4 py-1.5 rounded-md flex items-center gap-2" ><RiPagesLine size={18} /> Add Page</Link>
            </div>
            ...
            <div className="mt-4" >
                {!isLoading ? filteredPages?.map((page) => (
                    <Link to={`/pages/${page?._id}`} key={page?._id} className="border-b cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900 border-slate-200 dark:border-slate-800 md:transition-colors py-2 xs:px-2 flex justify-between items-center" >
                        <div className="flex gap-3 items-center">
                            <div className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 md:transition-colors" ></div>
                            <p className="capitalize text-lg font-semibold" >{page?.name}</p>
                        </div>
                        <div className="flex gap-2" >
                            <Link to={`/pages/edit/${page?._id}`} className="p-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded" ><MdEdit size={24} /></Link>
                        </div>
                    </Link>
                )) : <>
                    <PageSkeleton/>
